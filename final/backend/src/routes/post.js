...
const searchPost = async (req, res) => {
  ...
  if (post.length) res.status(200).send({ message: 'success', contents: post })
...
const createPost = async (req, res) => {
  ...
  if (origin) {
    ...
    try {
      ...
      res.send({
        message: `Updating (${title}, ${body}, ${author})`,
        post: origin,
      })
    } catch (err) {
      throw new Error(err)
...
const showPost = async (req, res) => {
  const posts = await PostModel.find().sort({ updated_at: -1 })

  if (posts.length)
    res.status(200).send({ message: 'success', contents: posts })
  else res.status(200).send({ message: 'error', contents: [] })
}
